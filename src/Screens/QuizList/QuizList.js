...
class QuizList extends Component {
  ...
  updateToPerfPage() {
    this.setState({ renderPerfPage: true });
  }

  // back button function
  back() {
    this.setState({ renderSelectedTestObj: false });
    this.setState({ renderPerfPage: false });
  }
  ...
  renderQuizInfo() {
    ...
    return (
      <div>
        <h2 className="text-center stylized">{saveSelectedQuizObj.quizName} Quiz</h2>
        <br/>
        ...
        <div className="row">
          {saveSelectedQuizObj.tests.map((test, i) => {
            return (
              ...
              >
                <div className="card better-card">
                  <img
    ...
    return (
      <div>
        <div className="row dash-header">
          <h1 className="hidden">Space</h1>
          <h1 className="stylized">Dashboard</h1>
          <button
            className="btn btn-performance"
            onClick={this.updateToPerfPage.bind(this)}
          >
            View Performance <i className="fa fa-forward" />
          </button>
        </div>
        <br/>
        <br/>
              ...
              <div className="col-md-4" key={`${qList}_${index}`}>
                <div className="card better-card">
                  ...
                  <div className="card-body quiz-card-body">
                    ...
                    <p className="card-text">
                      Test your skills or see much you know of {qList.quizName} by taking this small quiz.
  ...
  renderPerformanceList() {
    ...
    let filteredPerfList = []
    if(storedPerfList!=null){
      filteredPerfList = storedPerfList.filter(perfRecord => {
        if( perfRecord.username == this.props.currUsername ){
          return true;
        }
      });
    }
    ...
    return (
      <div>
        <h2 className="text-center stylized">Performance List</h2>
        <br/>
        <div className="row justify-content-center">
          <button className="btn btn-dark" onClick={this.back}>
            Back <i className="fa fa-backward" />
        ...
        {filteredPerfList.length==0 ? <div><br/><h1 className="text-center text-white">No Results</h1></div> : ''}
        
        <br/>
        <div className="row">
          {
            filteredPerfList.map((perfRecord, i) => {
              return (
                <div
                  className="col-md-4 col-sm-6"
                  key={`${perfRecord.quizName}_${perfRecord.name}_${i}`}
                >
                  <div className="card better-card">
                    <img
                      className="card-img-top"
                    ...
                    />
                    <div className="card-body quiz-card-body">
                      <h4 className="card-title">Test Taker: {perfRecord.username}</h4>
                      <h4 className="card-title">Score: {perfRecord.score}</h4>
                      <h5 className="card-title">{perfRecord.quizName} - {perfRecord.testName}</h5>
                      <p className="card-text">
                        Total Questions: {perfRecord.questions}<br/>
                        Total Time: {perfRecord.time / 60} Minutes
                      </p>
                    </div>
                  </div>
                </div>
